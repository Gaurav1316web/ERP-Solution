    ...
    Function ReturnQry()
        ...
        Try
            ...
            If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "AJM") = CompairStringResult.Equal Then
                ...
                ElseIf rbtnDispatch.IsChecked Then
                    whrcls = " And convert(date,TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) >= '" & clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") & "'   and convert(date,TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) <= '" & clsCommon.GetPrintDate(txtToDate.Value, "dd/MMM/yyyy") & "' "
                End If
            Else
                ...
                If ShowTodayDemandAsCurrentandUpcoming Then
                    ...
                    ElseIf rbtnDispatch.IsChecked Then
                        whrcls = " And convert(date,TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) >= '" & clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") & "'   and convert(date,TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) <= '" & clsCommon.GetPrintDate(txtToDate.Value.AddDays(1), "dd/MMM/yyyy") & "' "
                    End If
                Else
                    If rbtnDemand.IsChecked Then
                        whrcls = " And convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date,103) >= '" & clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") & "'   and convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date,103) <= '" & clsCommon.GetPrintDate(txtToDate.Value, "dd/MMM/yyyy") & "' "
            ...
            Else
                If rbtnMorning.IsChecked Then
                    ...
                    ElseIf rbtnDispatch.IsChecked Then
                        If clsCommon.GetPrintDate(txtFromDate.Value, "dd-MMM-yyyy") = clsCommon.GetPrintDate(txtToDate.Value, "dd-MMM-yyyy") Then
                            ...
                            whrShift = " and 2=( case when Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) >= Convert(Date, '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtToDate.Value), "dd/MMM/yyyy hh:mm tt") + "',103) and Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) <= Convert(Date, '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm tt") + "',103) and Shift_Type='PM' then 3 else 2 end  )"
                        Else
                            whrShift += " And Shift_Type = 'AM'  "
                            whrDCSSaleShift += " And Shift_Type = 'AM'  "
                        End If
                ...
                ElseIf rbtnEvening.IsChecked Then
                    ...
                    ElseIf rbtnDispatch.IsChecked Then
                        If clsCommon.GetPrintDate(txtFromDate.Value, "dd-MMM-yyyy") = clsCommon.GetPrintDate(txtToDate.Value, "dd-MMM-yyyy") Then
                            ...
                            whrShift += " and 2=( case when Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) >= Convert(Date, '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "',103) and Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) <= Convert(Date, '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "',103) and Shift_Type='AM' then 3 else 2 end  )"
                        Else
                            whrDCSSaleShift += " And Shift_Type = 'PM'  "
                            whrShift += " And Shift_Type = 'PM'  "
                        End If
                ...
                ElseIf rbtnBothShift.IsChecked Then
                    ...
                    ElseIf rbtnDispatch.IsChecked Then
                        If ShowTodayDemandAsCurrentandUpcoming Then
                            whrShift += " and 2=( case when Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) >= Convert(Date, '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "',103) and Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) <= Convert(Date, '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "',103) and Shift_Type='AM' then 3 else 2 end  )"
                            whrShift += " and 2=( case when Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) >= Convert(Date, '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtToDate.Value.AddDays(1)), "dd/MMM/yyyy hh:mm tt") + "',103) and Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) <= Convert(Date, '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value.AddDays(1)), "dd/MMM/yyyy hh:mm tt") + "',103) and Shift_Type='PM' then 3 else 2 end  )"
            ...
            End If
            If rbtnDispatch.IsChecked Then
                qry += "  SELECT 1 as SNO,max(TSPL_ITEM_MASTER.Short_Description)Short_Description, max(I.UOM_Code)UOM_Code, " & BaseQry & "  and TSPL_ITEM_MASTER.Is_FreshItem=1 and IsTaxable = 0 and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type <> 'MCC' group by TSPL_ITEM_MASTER.Item_Code  "
                If chkDCSSale.Checked Then
                    qry += "" & Environment.NewLine & " union all SELECT sno,max(Short_Description)Short_Description,max(UOM_Code)UOM_Code, max(Sku_Seq)Sku_Seq,sum(Evening_Qty) as Evening_Qty,0 as Morning_Qty,sum(Evening_Amt) as Evening_Amt,0 as Morning_Amt  ,sum(Total_Qty) as Total_Qty,isnull(sum(Total_Amt),0) as Total_Amt FROM ( SELECT 1 as SNO,max(TSPL_ITEM_MASTER.Short_Description)Short_Description, max(I.UOM_Code)UOM_Code,TSPL_ITEM_MASTER.Item_Code, TSPL_SD_SHIPMENT_HEAD.Supply_Date," & BaseQry2 & "  and TSPL_ITEM_MASTER.Is_FreshItem=1 and IsTaxable = 0  group by TSPL_ITEM_MASTER.Item_Code,TSPL_SD_SHIPMENT_HEAD.Supply_Date)xxx where 2=2  " + whrDCSSaleShift + "  group by Item_Code,sno "
                End If

                qry += "" & Environment.NewLine & " union all " & Environment.NewLine & "  SELECT 2 as SNO, 'Milk Total' as Short_Description,'' as UOM_Code, " & BaseQry & "  and TSPL_ITEM_MASTER.Is_FreshItem=1 and IsTaxable = 0  and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type <> 'MCC' "
                If chkDCSSale.Checked Then
                    qry += "" & Environment.NewLine & " union all " & Environment.NewLine & " SELECT max(sno)sno,max(Short_Description)Short_Description,max(UOM_Code)UOM_Code, max(Sku_Seq)Sku_Seq,sum(Evening_Qty) as Evening_Qty,0 as Morning_Qty,sum(Evening_Amt) as Evening_Amt,0 as Morning_Amt  ,sum(Total_Qty) as Total_Qty,isnull(sum(Total_Amt),0) as Total_Amt FROM (  SELECT 2 as SNO, 'Milk Total' as Short_Description,'' as UOM_Code,TSPL_SD_SHIPMENT_HEAD.Supply_Date, " & BaseQry2 & "  and TSPL_ITEM_MASTER.Is_FreshItem=1 and IsTaxable = 0 group by TSPL_SD_SHIPMENT_HEAD.Supply_Date)xxx where 2=2 " + whrDCSSaleShift + "  "
                End If

                qry += "" & Environment.NewLine & " union all " & Environment.NewLine & "  SELECT 3 as SNO, max(TSPL_ITEM_MASTER.Short_Description)Short_Description, max(I.UOM_Code)UOM_Code, " & BaseQry & " and (TSPL_ITEM_MASTER.Is_Ambient=1 or IsTaxable = 1 ) and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type <> 'MCC' group by TSPL_ITEM_MASTER.Item_Code  "
                If chkDCSSale.Checked Then
                    qry += "" & Environment.NewLine & " union all " & Environment.NewLine & " SELECT sno,max(Short_Description)Short_Description,max(UOM_Code)UOM_Code, max(Sku_Seq)Sku_Seq,sum(Evening_Qty) as Evening_Qty,0 as Morning_Qty,sum(Evening_Amt) as Evening_Amt,0 as Morning_Amt  ,sum(Total_Qty) as Total_Qty,isnull(sum(Total_Amt),0) as Total_Amt FROM ( SELECT 3 as SNO, max(TSPL_ITEM_MASTER.Short_Description)Short_Description, max(I.UOM_Code)UOM_Code, TSPL_ITEM_MASTER.Item_Code,TSPL_SD_SHIPMENT_HEAD.Supply_Date," & BaseQry2 & " and IsTaxable = 1  group by TSPL_ITEM_MASTER.Item_Code,TSPL_SD_SHIPMENT_HEAD.Supply_Date )xxx where 2=2  " + whrDCSSaleShift + " group by Item_Code,sno "
                End If
                qry += "" & Environment.NewLine & " union all " & Environment.NewLine & "  SELECT 4 as SNO, 'Product Total' as Short_Description,'' as UOM_Code, " & BaseQry & "  and (TSPL_ITEM_MASTER.Is_Ambient=1 or IsTaxable = 1 ) and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type <> 'MCC' "
                If chkDCSSale.Checked Then
                    qry += "" & Environment.NewLine & " union all " & Environment.NewLine & " SELECT max(sno)sno,max(Short_Description)Short_Description,max(UOM_Code)UOM_Code, max(Sku_Seq)Sku_Seq,sum(Evening_Qty) as Evening_Qty,0 as Morning_Qty,sum(Evening_Amt) as Evening_Amt,0 as Morning_Amt  ,sum(Total_Qty) as Total_Qty,isnull(sum(Total_Amt),0) as Total_Amt FROM ( SELECT 4 as SNO, 'Product Total' as Short_Description,'' as UOM_Code,TSPL_SD_SHIPMENT_HEAD.Supply_Date, " & BaseQry2 & "  and (TSPL_ITEM_MASTER.Is_Ambient=1 or IsTaxable = 1 ) group by TSPL_SD_SHIPMENT_HEAD.Supply_Date)xxx where 2=2  " + whrDCSSaleShift + " "
                End If
                If chkDCSSale.Checked Then
                    qry += " ) xxxx where SNO is not null and total_qty >0	group by sno,Short_Description		order by SNO, max(Sku_Seq ) "
                Else
                    qry += " ) xx	order by SNO, Sku_Seq "
                End If
            ElseIf rbtnDemand.IsChecked Then
                qry += "  SELECT 1 as SNO,max(TSPL_ITEM_MASTER.Short_Description)Short_Description, max(I.UOM_Code)UOM_Code, " & BaseQry & "  and TSPL_ITEM_MASTER.Is_FreshItem=1 and IsTaxable = 0 group by TSPL_ITEM_MASTER.Item_Code  " & Environment.NewLine & " union all " & Environment.NewLine & "  SELECT 2 as SNO, 'Milk Total' as Short_Description,'' as UOM_Code, " & BaseQry & "  and TSPL_ITEM_MASTER.Is_FreshItem=1 and IsTaxable = 0  
             " & Environment.NewLine & " union all " & Environment.NewLine & "  SELECT 3 as SNO, max(TSPL_ITEM_MASTER.Short_Description)Short_Description, max(I.UOM_Code)UOM_Code, " & BaseQry & " and (TSPL_ITEM_MASTER.Is_Ambient=1 or IsTaxable = 1 ) group by TSPL_ITEM_MASTER.Item_Code  " & Environment.NewLine & " union all " & Environment.NewLine & "  SELECT 4 as SNO, 'Product Total' as Short_Description,'' as UOM_Code, " & BaseQry & "  and (TSPL_ITEM_MASTER.Is_Ambient=1 or IsTaxable = 1 ) "
                qry += " ) xx	order by SNO, Sku_Seq "
            End If
